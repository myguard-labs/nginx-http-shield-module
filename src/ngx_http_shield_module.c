...
static ngx_int_t ngx_http_shield_check_dotfile(ngx_http_request_t *r,
    ngx_http_shield_loc_conf_t *slcf, ngx_http_shield_hit_t *hit);
/* The scanner itself lives in ngx_http_shield_scan.c, behind a seam that takes
 * bytes rather than a request -- see ngx_http_shield_scan.h. This wrapper is
 * the only place that knows both sides: it allocates the two scratch buffers
 * from r->pool and fails closed if it cannot, so the seam never has to. */
static ngx_int_t ngx_http_shield_scan_r(ngx_http_request_t *r, u_char *data,
    size_t len, const char *source, uint64_t skip,
    ...
    ngx_http_shield_loc_conf_t *slcf, ngx_http_shield_hit_t *hit)
{
    ...
    if (r->unparsed_uri.len) {
        ...
        rc = ngx_http_shield_scan_r(
                 r, r->unparsed_uri.data, r->unparsed_uri.len, "uri",
        ...
        {
            rc = ngx_http_shield_scan_r(
                     r, r->unparsed_uri.data, path_len, "uri",
                     slcf->skip | ~ngx_http_shield_no_query_mask(), hit);
            if (rc != NGX_DECLINED) {
                return rc;
            }
    ...
    for (i = 0; /* void */; i++) {
        ...
        rc = ngx_http_shield_scan_r(r, header[i].value.data,
...
/*
 * Request-side wrapper around the decision seam.
 *
 * The scanner needs two scratch buffers of `len` bytes each and has no opinion
 * about where they come from; here they come from r->pool. This is the whole
 * of what the seam gave up by not taking a request, and keeping it in one
 * function means there is exactly one place that can fail this way.
 *
 * A failed allocation returns NGX_ERROR, never NGX_DECLINED: the caller fails
 * closed, because reporting an unscanned buffer as clean is how a scanner
 * silently stops scanning under memory pressure.
 */
static ngx_int_t
ngx_http_shield_scan_r(ngx_http_request_t *r, u_char *data, size_t len,
    const char *source, uint64_t skip, ngx_http_shield_hit_t *hit)
{
    u_char  *scratch_lc, *scratch_dec;

    if (len == 0) {
        return NGX_DECLINED;
    }

    scratch_lc = ngx_pnalloc(r->pool, len);
    scratch_dec = ngx_pnalloc(r->pool, len);

    if (scratch_lc == NULL || scratch_dec == NULL) {
        return NGX_ERROR;
    }

    return ngx_http_shield_scan_input(data, len, scratch_lc, scratch_dec,
                                      source, skip, hit);
}
...
ngx_http_shield_scannable_body(ngx_http_request_t *r)
{
     ...
     * unlabeled -- so treat it as scannable rather than fail open. The SCAN
     * itself is bounded by shield_max_body (default 8k); the body is still
     * fully read by ngx_http_read_client_request_body up to
     * client_max_body_size before being truncated to that cap. */
    ...
    ngx_http_shield_loc_conf_t *slcf, ngx_http_shield_hit_t *hit)
{
    ...
    return ngx_http_shield_scan_r(r, body.data, body.len, "body",
                                  slcf->skip
...
ngx_http_shield_init(ngx_conf_t *cf)
{
     ...
     * matching signatures. */
    if (ngx_http_shield_ac_build(cf->pool, cf->log, &ngx_http_shield_ac_decoded,
    ...
    if (ngx_http_shield_ac_build(cf->pool, cf->log, &ngx_http_shield_ac_raw,
