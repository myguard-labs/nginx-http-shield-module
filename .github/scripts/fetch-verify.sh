#!/usr/bin/env bash
# sync-sha: 379703e9e2907527d49803cf99d2a8e1aebf873aea574b075bbfec07244895cb
...
# sha (used by compute-versions.sh to harvest fresh hashes for a version bump).
#
# If OUTFILE already exists with the right sha (warm actions/cache hit) the
# download is skipped -- the sha check still runs, so a poisoned cache is
# caught.
set -euo pipefail
